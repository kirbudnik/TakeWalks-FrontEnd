...
function formatCurrency(amount, currency) {
    ...
    //conversion is done in PaxPicker.updatePrice();
    //amount = amount * initValues.currency.exchangeRate;

    return initValues.currency.symbol + (( currency == 'USD' || currency == 'CAD' || currency == 'AUD' ) ? number_format(amount, 2, '.', ',') : number_format(amount, 2, ',', '.'));
}
console.log(initValues);
console.log(initValues.dates_private);
console.log(initValues.dates_private.length);
...
Tour = {
    ...
    BookingPanel: {
        ...
        reset: function (type) {
            ...
            //close time picker and guest picker
            Tour.BookingPanel.TimePicker.$el.parents('.time').slideUp();
            Tour.BookingPanel.PaxPicker.$el.slideUp();
        },
        ...
        showPrice: function (guests, price) {
            ...
            this.$el.find('p.summary').html(guests + ' ' + guestText + ' for <span class="item-content-price free total">' + formatCurrency(price, initValues.currency.type) + '</span>');
        },
        ...
        PaxPicker: {
            init: function (parent) {
                ...
                this.$el.find('select').on('change',this._changePeople);
            },
            _changePeople: function (e) {
                var guests = 0;
                var non_infant = 0;
                var type = $('input[name=type]').val();
                //var price = type == 'group' ? 0 : parseFloat($('fieldset.people').attr('data-private-base'));
                ...
                $.each(['adults', 'children', 'infants'], function (i, val) {
                    ...
                    for (var p = 0; p < g; p++) {
                        ...
                        //price += parseFloat(select.attr('data-'+type+'-price'));
                        if (prices[val]) {
                    ...
                    if (val != 'infants') {
                        //removing a category causes g to be NaN
                        non_infant += g;//non_infant += isNaN(g) ? 0 : g;
                    }
                });

                if (!non_infant) {
                    Tour.BookingPanel.PaxPicker.parent.hideSection('book');
                    return;
                }

                Tour.BookingPanel.PaxPicker.parent.setSummaryText('people', guests + ' Guest' + (guests != 1 ? 's' : ''));
                Tour.BookingPanel.PaxPicker.parent.showSection('book');
            ...
            render: function () {
                ...
                $.each(['adults', 'children', 'infants'], function (i, val) {
                    $('span.item-content-price.' + val).html(formatCurrency(prices[val], 'EUR'));
                });
            ...
            updatePrice: function () {
                ...
                var type = $('input[name=type]').val();
                var amt = 0;
                if (Tour.BookingPanel.private_base_price_update === undefined) {
                    amt = parseFloat(Tour.BookingPanel.private_base_price) * initValues.currency.exchangeRate;
                    Tour.BookingPanel.private_base_price = amt;//number_format(amt, 2, '.', ',');
                    Tour.BookingPanel.private_base_price_update = true;
                }
                if (!date && !time) return true;
                var prices = null;
                if (type == 'group') {
                    if (Tour.BookingPanel.dates_group[date] && Tour.BookingPanel.dates_group[date][time] && Tour.BookingPanel.dates_group[date][time].currecyUpdated) return true;
                    prices = Tour.BookingPanel.dates_group[date][time].prices;
                } else if (type == 'private') {
                    if (Tour.BookingPanel.dates_private[date] && Tour.BookingPanel.dates_private[date][time] && Tour.BookingPanel.dates_private[date][time].currecyUpdated) return true;
                    prices = Tour.BookingPanel.private_prices;
                }
                for (var p in prices) {
                    amt = parseFloat(prices[p]) * initValues.currency.exchangeRate;
                    prices[p] = number_format(amt, 2, '.', ',');
                }
                if (prices && type == 'group') {
                    Tour.BookingPanel.dates_group[date][time].prices = prices;
                    Tour.BookingPanel.dates_group[date][time].currecyUpdated = true;
                } else if (prices && type == 'private') {
                    //private
                    Tour.BookingPanel.private_prices = prices;
                    Tour.BookingPanel.dates_private[date][time].prices = prices;
                    Tour.BookingPanel.dates_private[date][time].currecyUpdated = true;
                }
        ...
        TimePicker: {
            ...
            render: function () {
                ...
                this.$el.html(html).val(this.$el.find('option:first').attr('value')).dropdown("update");
            ...
            _selectTime: function (e) {
                ...
                //show or hide base price private message
                $('.book .private-details').css('display', $('.book input[name=type]').val() == 'private' ? 'block' : 'none');
            }
        },
...
$(document).ready(function ($) {
    ...
    Tour.Slider.init();
    $('header.clearfix').attr('id', 'top');

    // Group/private tabs
    $('form .aside-content .nav-tab').click(function () {
        $('form .aside-content .nav-tab').removeClass('active');
        ...
        if (type == 'private') {
            ...
            $('.people .private-details').show();
        } else {
            $('.people .private-details').hide();
        }
    ...
    function showVideo() {
        var pl = $('.player');
        //var player = new VimeoPlayer(document.getElementById('banner-video'));

        //PLAY BTN
        $('.play-button').click(function(){
            pl.stop(1,1).fadeIn(800);

            pl.find('#banner-video').attr('src', pl.find('#banner-video').data('video-link'));
