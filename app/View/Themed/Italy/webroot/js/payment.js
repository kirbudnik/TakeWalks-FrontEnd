...
var Payment = {
    ...
    formatTourPrices: function(tour){
        ...
        if(tour.discount_bundle_tour_percent > 0){
            ...
            tour.discountBundleTour = Payment.formatPrice((subtotal * discountPercent).toFixed(2));
    ...
    removeItem : function(event){
        ...
        //prevents that a double click remove next tour
        $('.close.fa.fa-remove').remove();
        $('body').css({'cursor':'wait'});
        $.get( event.target.dataset.href, function(data) {
            initValues.cart = data.cart_payment;
            initValues.promo_discount_fixed_total = data.promo_discount_fixed_total;
            $('.cart_count').html(initValues.cart.length);
            Payment.loadCart();
            WrapperGA.ecRemoveFromCart(event.target);
            $('body').css({'cursor':''});
        });
    },
