...
<section id="content" class="columns-c payment-form" data-full-price-euro="<?php echo $full_price ?>" data-full-price-usd="<?php echo $full_price_usd ?>">
    ...
    <aside>
                        ...
                        <?php foreach(array('adults', 'seniors', 'students', 'children', 'infants') as $type): ?>
                            <?php if($item[$type]) : ?>
                                <li>
                                    ...
                                    <span><?php echo $item["{$type}_price"] == 0 ? 'FREE' : ExchangeRate::convert($item["{$type}_price"],1,1,'EUR') ?> (<?php echo $item["{$type}_price"] == 0 ? 'FREE' : ExchangeRate::convert($item["{$type}_price"],1,1,'USD') ?>)</span>
                                </li>
                            <?php endif ?>
                        <?php endforeach; ?>
                        <li class="strong">Subtotal <span><?php echo $item['total_price'] == 0 ? 'FREE' : ExchangeRate::convert($item['total_price'],1,1,'EUR') ?> (<?php echo ExchangeRate::convert($item['total_price'],1,1,'USD');?>)</span></li>
                        <?php if(isset($item['promo_local'])): ?>
                            <li>Discount <span>- <?php echo $item['promo_discount'] == 0 ? 'FREE' : ExchangeRate::convert($item['promo_discount'],1,1,'EUR') ?> (<?php echo ExchangeRate::convert($item['promo_discount'],1,1,'USD');?>)</span></li>
                        <?php endif ?>
                    </ul>
        ...
        <form method="post" action="/pages/apply_promo" class="form-d" id="apply_promo_form">
            <fieldset>
                <h3><label for="fda"><a id="add_promo">Add promo code or gift card</a></label></h3>
    ...
    <form method="post" class="form-c" novalidate id="travellerInfo"  onsubmit="return WrapperGA.ecCheckout();">
...
<script type="template" class="cartTour">
    <article class="tour">
        ...
        <ul>
            ...
            <% } %>
            <% if(discount_bundle_tour_percent > 0 || discount > 0 ){ %>
            <li style="text-decoration: line-through">Original price <span style="text-decoration: line-through"><%= originalPrice %></span></li>
            <% } %>
            <% if(discount_bundle_tour_percent > 0){ %>
            <li>Discount <span>- <%= discountBundleTour %></span></li>
            <% } %>
            <% if(discount > 0 ){ %>
            <li>Discount promo code <span>- <%= discountAmount %></span></li>
            <% } %>
            <li class="strong">Subtotal <span><%= priceDiscount %></span></li>
        </ul>
        <a 
        data-position="<%- position %>" 
        data-price="<%- totalPrice * (1 - (discount_bundle_tour_percent / 100)) %>"
        data-name="<%- name %>" 
        data-id="<%- event_id %>"
        data-sku="<%- sku %>"
        data-href="/pages/remove_from_cart/<%-position %>"
        onclick="Payment.removeItem(event)"
        href="#"
        class="close fa fa-remove" style="text-decoration: none"></a>
    </article>
</script>
