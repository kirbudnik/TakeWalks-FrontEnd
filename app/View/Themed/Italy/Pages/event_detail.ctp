...
$sameInCart = false;
$type = ($event['group_private'] == 'Private') ? 'private' : 'group';
$discount = 1 - ($discountPercentRelatedTour / 100);
$adultPrice = $event['adults_price'];
$adultPriceDiscount = round($adultPrice * $discount, 2);
foreach($cart as $i => $item){
    $itemsRelated = $item['related'];
    if($item['event_id'] == $event['id']){
        $sameInCart = true;
    }
    if (in_array($event['id'], $itemsRelated)){
...
<header id="top-fixed" class="top-detail-nav">
    <div class="event-title">
        <div class="container">
            <div style="float: left;width: 40px;"><a href="/">Walks of Italy</a></div>
            <a href="#book" class="book-tour-anchor">Book Tour</a>
            <a href="#" style="float: right;width: 160px;"><h5>View Tours</h5></a>
            <h5><?php echo $event['name_listing'] != '' ? $event['name_listing'] : $event['name_short'] ?></h5>
        </div>
    </div>
...
<div itemscope itemtype="http://schema.org/Product">
    ...
    <nav id="breadcrumbs">
        <h1 itemprop="name"><?php echo $event['name_long'] ?></h1>
        <ol>
            <li>
        ...
        <p>
            <?php if ($showDiscount): ?>
                <span class="triangle-discount-detail"></span>
                <span class="triangle-discount-content">
                <span class="save">SAVE</span><br>
                <span class="price"><?php echo $discountPercentRelatedTour; ?>%</span>
            </span>
                <span class="discount-detail">
                <span class="from">FROM</span><br>
                <span class="price"><?php echo ExchangeRate::convert(ceil($adultPrice), false); ?></span>
            </span>
                ...
                <span style="margin-bottom: -10px;"><?php echo $event['group_private'] != 'Private' ? 'From' : 'Price for 2 people' ?>
                </span>

                <meta itemprop="priceCurrency" content="EUR"/>
                <b style="display:inline;font-weight:500" itemprop="price" content="<?php echo $adultPrice ?>">
                    <?php echo ExchangeRate::convert(ceil($adultPriceDiscount), false)?>
                </b>
                <link itemprop="availability" href="http://schema.org/InStock"/>


            </span>
                <span class="text-discount">when booked with </span><br>
                <span class="text-discount last">the tour in the cart</span>


                <a href="#book-now" class="book_now">BOOK NOW</a>

                <span class="rating-a <?php echo isset($ratings[$event['id']]) ? $ratings[$event['id']]['letterClass'] : ''?>" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating"
                      style="<?php echo !isset($ratings[$event['id']]) ? 'display:none;' : ''?>  margin-left: 30px;    top: 5px;">
            ...
            <?php else : ?>
                ...
                <link itemprop="seller" href="seller_info"/>
                <span><?php echo $event['group_private'] != 'Private' ? 'From' : 'Price for 2 people' ?>
                </span>
    ...
    <article class="event-detail-hero">
        ...
        <div class="extra-expand">
            ...
            <!--            <figure style="background-image: url('<?php echo $images[0]['images_name'] ?>')"></figure>-->
        </div>
    </article>

    <?php if($charity && $charity['description'] && $charity['video']): ?>
        <?php
        //echo $this->element('Pages/detail/positive_steps',array('description' => $charity['description'], 'video' => $charity['video']));
        ?>
    <?php endif ?>
    ...
    <section id="content">
        <div class="columns-a no-match-height">
            <article>
                ...
                <ul class="list-triangles">
                    <li><h3><?php echo $event['bullet1'] ?></h3></li>
                    <li><h3><?php echo $event['bullet2'] ?></h3></li>
                ...
                <footer class="double-a a activity-info-below-slider">
                    <div>
                        <h4 class="header-a" id="sites-visited">Sites visited</h4>
                        <ul class="list-d">
            ...
            <aside>
                ...
                <div class="calendar-a">
                    <form class="book" action="/add_to_cart" method="post" onsubmit="return WrapperGA.ecAddToCartBookingForm();" id="booking_form">
                        ...
                        <input type="hidden" name="show_discount" value="<?php echo $showDiscount; ?>" />
                        <h4 id="book">Book a tour</h4>
                        ...
                        <fieldset class="submit">
                            ...
                            <button type="submit" class="green button large" id="btn_book_now"><i class="fa fa-cart-plus"></i>BOOK NOW</button>
                        </fieldset>
                    </form>

                </div>
                <figure class="scheme-c">
                    <a href="http://www.tripadvisor.ie/Attraction_Review-g187791-d1838073-Reviews-Walks_of_Italy-Rome_Lazio.html">
                        <img src="/theme/Italy/img/content/tripadvisor.png" alt="Trip Advisor Certificate of Excellence 2011-2014" width="238" height="241">
                    </a>
                </figure>
            </aside>
        </div>
        ...
        <?php echo $this->element('Pages/detail/tour-bundle') ?>

        <?php if(!empty($relatedTours) && 1==2): ?>
...
<?php
if (!CakeSession::check('promoCodeUrl')) {
    echo $this->element('Pages/detail/detail-cart-modal');
}
?>
