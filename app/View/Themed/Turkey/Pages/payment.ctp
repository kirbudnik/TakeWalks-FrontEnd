...
<section id="content" class="columns-c payment-form">
    <aside style="margin: 80px 20px 0 0;">
                        ...
                        <?php foreach(array('adults', 'seniors', 'students', 'children', 'infants') as $type): ?>
                            <?php if($item[$type]) : ?>
                                <li>
                                    ...
                                    <span><?php echo $item["{$type}_price"] == 0 ? 'FREE' :  ExchangeRate::convert($item["{$type}_price"], 1,1,'USD') ?></span>
                                </li>
                            <?php endif ?>
                        <?php endforeach; ?>
                        <li class="strong">Subtotal <span><?php echo $item['total_price'] == 0 ? 'FREE' : ExchangeRate::convert($item['total_price'],1,1,'USD') ?></span></li>
                        <?php if(isset($item['promo_local'])): ?>
                            <li>Discount <span>- <?php echo ExchangeRate::convert($item['promo_discount'],1,1,'USD');?></span></li>
                        <?php endif ?>
                    </ul>
                    <a 
                        data-position="<?php echo $n ?>" 
                        data-price="<?php echo $item['total_price']  ?>" 
                        data-name="<?php echo $item['name'] ?>" 
                        data-id="<?php echo $item['event_id'] ?>" 
                        data-href="/pages/remove_from_cart/<?php echo $n ?>" 
                        onclick="ecRemoveFromCart(this);"
                        href="#" class="close" href=""><i class="fa fa-remove"></i></a>
                </article>
            <?php endforeach ?>
            -->
        ...
        <form method="post" action="/pages/apply_promo" class="form-d">
            <fieldset>
                <h3><label for="fda"><a id="add_promo">Add promo code</a></label></h3>
...
<script type="template" class="cartTour">
    <article class="tour">
        ...
        onclick="ecRemoveFromCart(this);"
        href="#"
        class="close" ><i class="fa fa-remove"></i></a>
    </article>
</script>
