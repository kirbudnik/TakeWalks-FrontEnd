...
URL of the web page where the tag is expected to be placed: https://www.walksofturkey.com/confirm
            ...
            <table>
                <?php foreach(CakeSession::read('charitiesDonatedTo') as $charityName => $donationAmount): ?>
                    <tr>
                        ...
                        <td>$<?php echo number_format($donationAmount * $exchangeRate,2,'.',','); ?></td>
                    </tr>
                <?php endforeach ?>
            </table>
        <?php endif ?>
        <?php $donationAmount = isset($donationAmount) ? $donationAmount : 0; ?>
...
<?php echo $this->element('google_conversion',array(
    'domain' => 'turkey',
    'item_price' => number_format(ExchangeRate::convert($total_price, 'USD', false) + (isset($donationAmount) ? $donationAmount : 0),2)
)) ?>
