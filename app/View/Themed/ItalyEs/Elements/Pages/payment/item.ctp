<div>
    ...
    <ul class="ticketsSummary">
        <?php foreach(array('adults', 'seniors', 'students', 'children', 'infants') as $type) : ?>
            <?php if($item[$type]) : ?>
                ...
                <li>
                    ...
                    <span class="<?php echo $price ? '' : 'gratis' ?>">
                        <?php echo $price > 0 ? ExchangeRate::format($price, $currency) : 'GRATIS' ?>
                    </span>
                </li>
            <?php endif ?>
        <?php endforeach ?>
        ...
        <?php if(isset($item['promo_local'])): ?>
            <li>
                <strong style="text-decoration: line-through">Precio original</strong>
