...
var Payment = {
    ...
    loadCart: function(){
        ...
        //update total price
        var tprice = Payment.formatPrice(Number(Payment.totalPrice).toFixed(2));
        var arrFormat = [{str:'&#36;', val: '$'} , {str:"&#8378;" , val : '₺'} , {str: "&#8364;", val : '€'} , {str : "&#163;", val : '£'}];
        for(var i in arrFormat){
            if(tprice.indexOf(arrFormat[i].str) > -1){
                tprice = tprice.replace(arrFormat[i].str, arrFormat[i].val);
            }
        }

        if(this.isGiftCardPage) {
            $("#submit_booking").val("Submit");
        } else {
            Payment.el.sideBar.find('.fullPrice').html(tprice);
            $("#submit_booking").val("Submit booking - " + tprice);
        }
    },
...
$(document).ready(function(){
	...
	$('.fname input, .lname input').blur(function(){
		$('.ccName input[name=ccFirstName]').val($('.fname input').val());
        $('.ccName input[name=ccLastName]').val($('.lname input').val())
	});
  ...
  $('form#travellerInfo').validate({
      ...
      submitHandler: function(form) {
          ...
          if($('select[name="ccType"]').val() == ''){
              $('span#conditionError', form).html('<br />Please choose a Card Type.');
              return false;
          }

          if($('select[name="ccMonth"]').val() == 'MM' || $('select[name="ccYear"]').val() == 'YYYY'){
              $('span#conditionError', form).html('<br />Please set the correct Expiry date.');
              return false;
          }
		  ...
		  if($('input[name="email"]').val() != $('input[name="confirm_email"]').val()){
			  $('span#conditionError', form).html('<br />Your confirm email does not match the original one.');
              return false;
		  }
