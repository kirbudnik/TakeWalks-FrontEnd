...
foreach($cart as  $n => $item) {
    ...
    $full_price_usd += $cart[$n]['total_price'];
}
?>
...
<div class="content wrap">
	<aside>
        ...
        <?php $total_price = 0; $restrictions = array();?>
            <!--
        <?php foreach($cart as $n => $item) : ?>
            <?php if($item['restrictions_caption'] )
                $restrictions[$item['event_id']] = $item['restrictions_caption'];
            ?>
            <?php $total_price += isset($item['promo_local']) ? floatval($item['promo_local']) : floatval($item['total_price'] ?: 0) ?>
            <?php echo $this->element('Pages/payment/item', compact('item','n')); ?>
            <br/>
        <?php endforeach ?>
            -->
        <ul class="ticketsSummary total">
            ...
            <li class="large">
                ...
                <!--<span><?php echo $this->Event->formatCurrency($total_price); ?></span>-->
            </li>
        </ul>
            
        <?php if(isset($promo_code)): ?>
            <span>Promo Code: <?php echo $promo_code ?></span>
            <a href="/pages/remove_promo_code" class="close" style="font-size: 75%">(clear code)</a>
        <?php else: ?>
            <form action="/pages/apply_promo" method="post" class="promo clearfix" id="promoForm">
                <label for="promo">Add promo code</label>
                <input type="text" name="promo">
                <input type="hidden" name="infoBlob">
                <input type="submit" class="pink button" value="Apply">
            </form>
            <script type="text/javascript">
                $(document).on('ready', function() {
                    $('#promoForm').on('submit', function(e) {
                        $(this).find('input[name=infoBlob]').val($('#travellerInfo').serialize());
                    })
                })
            </script>
        <?php endif; ?>
	...
	<main class="serif">
        ...
        <form id="travellerInfo" method="post" novalidate onsubmit="return ecCheckout();">
            ...
            <fieldset>
                ...
                <label class="phone">
                    Phone Number
                    <input type="text" name="phone_number" value="<?php if(isset($_POST['phone_number'])) echo $_POST['phone_number'] ?>" required maxlength="14" placeholder="Cell number that you will have on the day of the tour">
                </label>
...
<!--                        <option value="GB" --><?php //if(isset($_POST['country']) && $_POST['country'] == 'GB') echo 'selected' ?><!-->Wales</option>-->
                ...
                <?php if($restrictions): ?>
                    <?php foreach($restrictions as $n => $caption): ?>
                        <label class="country">
                            <?php echo $caption ?><br/>
                            <input type="text" name="restrictions[<?php echo $n ?>]" >
                        </label>
                    <?php endforeach; ?>
                <?php endif;?>
            </fieldset>
...
<!--            --><?php //endif ?>
            ...
            <input class="large pink button" id="submit_booking" type="submit" value="Submit booking - <?php echo $this->Event->formatCurrency($total_price) ?>">
        </form>
